...
- 🟠 **Text knowledge fixes nothing for Qwen3-VL, but substantially helps MedGemma** — F1 0.000→0.000 vs. 0.242→0.719 / 0.568→0.792 (p<0.0001 both, reproduced exactly on a second run). Text can't compensate for *zero* perception, but sharpens *partial* perception — details and verification in *Honest limitations*.
...
- 🟣 **Domain pretraining and few-shot exemplars stack, not substitute** — MedGemma-4B with the *same* exemplars reaches F1 0.840–0.857 (p<0.0001 vs. both MedGemma zero-shot and Qwen3-VL+exemplars, both datasets).
- 🟤 **A "smarter" exemplar-selection trick (hard negatives) backfires** — underperforms plain retrieval on both models and datasets, via lower recall only. Traced to one cause: the negative's mere *presence*, not its difficulty or position — and it hurts the general model far more than the domain-pretrained one. Full 4-way mechanism ablation in *Honest limitations*.
...
*Captured from a real EC2 run (2026-06-26). Content is genuine `docker build`/`docker run`/`curl` output.*
...
- **Text knowledge cannot substitute for visual training — for a model that has none to begin with.** Injecting textbook descriptions of each hemorrhage subtype (retrieval-augmented generation) left Qwen3-VL's floor-level result unchanged on CT-ICH (F1=0.000 either way) and within the same confidence interval as no-RAG on RSNA (0.202 vs 0.158, CIs overlap). For *this* model, the gap is perceptual, not a missing-knowledge problem — which is why a text-knowledge fix doesn't move it. (The next bullet shows this doesn't generalize to a model with partial visual competence already.)
- **A smarter retrieval query doesn't change that conclusion either.** Text-RAG's fixed query (`DEFAULT_QUERY`, retrieving the whole 5-document KB) could be dismissed as "too naive a query" rather than evidence that text knowledge itself can't help. We tested that alternative explanation directly: a two-stage variant has the model describe its own findings first, then retrieves only the top-2 KB entries conditioned on that description, before answering. Result: statistically indistinguishable from the original fixed-query Text-RAG on both datasets (paired-bootstrap diff=0.000, p=1.0 on CT-ICH and RSNA alike — 147–148/150 predictions were literally identical to Text-RAG's). This rules out "the query was too naive" as the reason text retrieval doesn't fix the perception gap, for Qwen3-VL.
- **Surprising result, checked twice: the same text injection that does nothing for Qwen3-VL substantially helps MedGemma.** We had assumed (without testing) that the "text can't fix it" conclusion above would carry over to MedGemma, since both models share the same Text-RAG code path and KB. It doesn't. MedGemma-4B + Text-RAG reaches F1=0.719 (CT-ICH, +0.477 over its own zero-shot, p<0.0001) and F1=0.792 (RSNA, +0.224, p<0.0001). This isn't a "say yes more" artifact: precision barely moves (CT-ICH 0.789→0.793; RSNA 0.977→0.874, a small drop) while recall does almost all the work (CT-ICH 0.143→0.657; RSNA 0.400→0.724). Because the result was surprising, we deleted the output and re-ran both datasets end-to-end a second time before trusting it (generation is greedy/deterministic, `do_sample=False`, so a literal re-run only catches execution-side flukes, not sampling noise) — both runs produced byte-identical raw model output, 0/150 differences on either dataset. Best current reading: text knowledge can't compensate for *zero* visual perception (Qwen3-VL), but can substantially sharpen *partial* visual perception (MedGemma already scores non-trivially zero-shot) by giving the model explicit criteria to apply to what it's already seeing. We hadn't tested this combination until asked to — it was an inherited, untested assumption, not a verified one.
